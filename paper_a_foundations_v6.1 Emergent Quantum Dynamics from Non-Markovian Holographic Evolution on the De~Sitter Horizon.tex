...
\emph{Program architecture.} This work is part of a broader program organized in four layers: (\emph{A})~an empirical low-acceleration regularity in gravitating systems; (\emph{B})~an effective nonlocal or retarded phenomenology that may account for it; (\emph{C})~a possible horizon-based geometric or informational interpretation of that phenomenology; and (\emph{D})~a still-incomplete attempt at a unified microscopic description connecting the boundary to the bulk. The present paper proposes the microscopic mechanism connecting Layers~C and~D---boundary indivisibility projected holographically---and derives from it Layer~A consequences tested in companion papers. Its claims should be read at that level: the mechanism is a proposal, the Layer~A consequences are quantitative predictions, and Layers~B and~C remain better established than Layer~D.
